...
def main(filename: str,output_res: tuple):
    ...
    with tqdm(total = (ny * nx)/4) as pbar:
        for j in range(ny-1 ,0,-2):
            for i in range(0,nx,2):
                col = vec3((0,0,0))
                for s in range(0,num_samples):
                    u = np.array([float(i + random.random())/float(nx),float(i + 1 + random.random())/float(nx)])
                    v = np.array([float(j + random.random())/float(ny),float(j + 1 + random.random())/float(ny)])
                    # u = float(i + random.random())/float(nx)
                    # v = float(j + random.random())/float(ny)
                    r = cam.get_ray(u,v)
                    #p = r(2.0)
                    # print(color(r,hit_object_list,0))
                    # print(col)
                    col += color(r,hit_object_list,0)
                pbar.update(1)
                col /= float(num_samples)
                col = vec3((np.sqrt(col[0]),np.sqrt(col[1]),np.sqrt(col[2])))
                ir = 255.99 * col[0];
                ig = 255.99 * col[1];
                ib = 255.99 * col[2];
                #print(output[i:i+2,j:j+2,:].squeeze().shape)
                #print(np.expand_dims(np.array([ir,ig,ib]).T,axis=1).shape)
                output[i:i+2,j:j+2,:] = np.expand_dims(np.array([ir,ig,ib]).T,axis=1)
